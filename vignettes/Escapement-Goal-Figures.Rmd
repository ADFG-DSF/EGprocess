...
# Replace with an age composition example
p_Igushik <- get_age(run_data = data_Igushik,
                      min_age = 3,
                      max_age = 7)
...
Some `EGprocess` functions utilize the function argument `profile_data` which is a derivative of the `posterior data` files described above and are required to build optimal yield profiles. A helper function is provided to create `profile_data` from `posterior_data`. 

```{r}
# For a single posterior
profile_Igushik_byr63_15 <- 
  get_profile(posterior_list = post_Igushik_byr63_15,
              multiplier = 1e-5)

#For multiple posteriors
...
```{r}
output_Igushik <-
  output_SR(posterior_list = post_Igushik_list, 
...
#Save the parameter table as a png file
save_as_image(output_Igushik[[5]],
              path = "../man/figures/Igushik_parameters.png")
```
...
![Figure 5: Igushik River sockeye salmon spawner-recruit parameters.](../man/figures/Igushik_parameters.png)
...
First time  escapement goals can be accommodated by creating a `goal_data` file that reflects that status. 
...
```{r}
# Made this to illustrate a point but it does be the question of whether goal type should be an column in the goal_data.
...
EGPIT decided escapement goals intended to target MSY should use a universal standard target yield of greater than 90% of MSY. A standard target of 90% of MSY makes sense for most salmon fisheries in Alaska. Arguably, Igushik River sockeye salmon is not one of them. The first issue is practical: because the lower bound of the escapement goal offers near 0% probability of achieving 90% of MSY, the OYP in Figure 4 does a poor job of discriminating between potential changes to the lower bound of the escapement goal. Taken alone, that might be acceptable. However, Figure 4 demonstrates the the current goal is sub optimal with respect to a target of 90% of MSY and makes it clear that reducing the lower bound of the escapement goal further would be even less optimal. However, Bristol Bay stakeholders have expressed a preference for suboptimal yield because they prefer fisheries that open regularly, and because the fishery harvests can easily exceed processor capacity even with suboptimal yield[^4]. In situations like this, where staff and stakeholders can clearly describe a need or preference to target suboptimal yield, profile plots can be modified to evaluate escapement goals relative to both optimal and suboptimal yield. This can be achieved by specifying the suboptimal yield level to the function `get_profile`. *(Note to EGPIT: We should decide if this functionality is needed.)*
...
# 
# Igushik_profile_80 <-
#   plot_profile_facet(profile_data = profile_Igushik_80,
